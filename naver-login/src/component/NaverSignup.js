...
function NaverSignup() {
  ...
  const 회원가입기능 = () => {
    // 비밀번호 비어있으면 비밀번호를 입력해달라고 띄우기
    if (!password) {
      // !password는 비밀번호가 없다는 뜻.
      alert("비밀번호를 입력해주세요.");
      return;
    }
    ...
    axios
      .post("http://localhost:9010/naverAPI/register", {
    ...
    <>
      ...
      {userInfo ? (
        <div>
          ...
          <input type="text" value={userInfo.response.ninkname} disabled />
          <input type="text" value={userInfo.response.name} disabled />
          <input
            type="image"
            src={userInfo.response.profile_image}
            disabled
            width="20%"
          />
