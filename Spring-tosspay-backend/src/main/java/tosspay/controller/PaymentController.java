 ...
 * 
 * 
 * HttpEntity : HTTP 요청 또는 응답의 본문(body)과 헤더(headers)를 포함하는 객체 HTTP 요청을 보낼 때 본문과
 * 헤더를 설정하고자 할 때 사용 본문(body) : 실제 전송될 데이터 ex) 아이디 비밀번호 작성한 글 등 헤더(headers) :
 * HTTP 헤더 정보를 포함 ex) 글자인지 이미지인지 동영상인지 .. 어떤 파일이 들어오는 것인가? 누가보내나?
 * HttpEntity<문자열이면 문자열, 숫자면 숫자, 모르겠으면 비워두기> abc = new HttpEntity<비워져있어도 상관
 * 없음>("요청 본문", headers);
 * 
 * 
 * ResponseEntity : 사용자가 요청한 응답을 개발자가 다시 사용자한테 전달할 때 사용
 * 
 * ResponseEntity (Response = 응답 / Http를 상속받아서 Http 기능에 응답에 대한 기능을 추가로 설정한
 * Entity) : HttpEntity를 상속받아, Http 응답에 대한 추가적인 정보를 제공 상태코드를 포함하고 있어서
 * 클라이언트(사용자)에게 응답을 보낼 때 사용 ResponseEntity<String이면 String, Integer면 Integer
 * 여러값이면 ?, 모르겠으면 비워두기> = new ResponseEntity<>("응답본문",headers);
 * 
 * ReQuestEntity (Request = 요청 / Http를 상속받아서 Http 기능에 요청에 대한 기능을 추가로 설정한 Entity)
 * : HttpEntity를 상속해, Http 요청에 대한 추가적인 정보를 제공 URI와 HTTP메서드
 * (GET,POST,PUT,DELETE)를 포함하고 있어, 서버로 요청을 보낼 때 주로 사용 RequestEntity<String이면
 * String, Integer면 Integer, 여러값이면 ?, 모르겠으면 비워두기> = new
 * RequestEntity<>("요청본문",headers); RequestEntity<String> req = new
 * RequestEntity<>("요청본문",headers, HttpMethod.POST, url);
 * 
 * 차이점 요약하기 클래스 상속관계 주요 사용 목적 추가 정보 HttpEntity 기본 클래스 HTTP 요총/응답 본문과 헤더 포함
 * 상태코드없음(성공여부) ResponseEntity HttpEntity상속 HTTP 응답 반환 상태코드포함(성공여부)
